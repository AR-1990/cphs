...
// dd($data['entryId']);
    ...
    public function getDashboardStats()
    {
        ...
        foreach ($schoolTurnoverRows as $sch) {
            ...
            $turnoverRaw = $sch->turnover;
            $turnoverDay = 1;
            $turnoverMonth = null;
            if (!is_null($turnoverRaw) && $turnoverRaw !== '') {
                if (preg_match('/^(\d{1,2})-(\d{1,2})$/', $turnoverRaw, $m)) {
                    $turnoverDay = (int)$m[1];
                    $turnoverMonth = (int)$m[2];
                } else {
                    $turnoverMonth = (int)$turnoverRaw;
                }
            }

            if ($turnoverMonth && $turnoverMonth >= 1 && $turnoverMonth <= 12) {
                $now = \Carbon\Carbon::now();
                $anchorThisYear = \Carbon\Carbon::createFromDate($now->year, $turnoverMonth, $turnoverDay)->startOfDay();
                if ($anchorThisYear->greaterThan($now)) {
                    $cycleStart = $anchorThisYear->copy()->subYear()->startOfDay();
                } else {
                    $cycleStart = $anchorThisYear->copy()->startOfDay();
                }
                ...
                $res = DB::table('form_entries')
                    ...
                    ->select(
                        ...
                        DB::raw('COUNT(CASE WHEN form_data.key IN ("Question_No_31_Conclusion_of_hearing_test_with_Rinner_and_Weber", "Question_No_28_Conclusion_of_hearing_test_with_Rinner_and_Weber") AND form_data.value IN ("right_ear_conductive_hearing_loss","left_ear_conductive_hearing_loss","right_ear_sensorineural_hearing_loss","left_ear_sensorineural_hearing_loss") THEN form_entries.id END) as RinnerWeberCount'),
                ...
                foreach ($res as $school) {
                    $bmi_moderate = $resBMI->bmi_moderate ?? 0;
                    $bmi_severe = $resBMI->bmi_severe ?? 0;
                    $rowTotal = (
                        $school->NormalPostureCount +
                        $school->MentalStatusCount +
