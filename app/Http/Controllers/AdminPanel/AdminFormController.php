    ...
    public function GeneralInfo(Request $request, $id = null)
    {
        ...
        $data['details'] = [
            ...
            "referred_to" => $resultArray[0]['referred_to'] ?? null,
             "psychological_comment" => isset($resultArray[0]['social_emotional-test']) && !empty($resultArray[0]['social_emotional-test']) ? str_replace("No.", "", $resultArray[0]['social_emotional-test']) : (isset($resultArray[0]['Psychologist_Findings']) && !empty($resultArray[0]['Psychologist_Findings']) ? str_replace("No.", "", $resultArray[0]['Psychologist_Findings']) : (isset($resultArray[0]['psychological_comment']) && !empty($resultArray[0]['psychological_comment']) ? str_replace("No.", "", $resultArray[0]['psychological_comment']) : null)),
...
// lead expouser
  // lead exposure
        ...
        $gender = $genderRaw;
        if ($genderRaw === 'f' || $genderRaw === 'girl' || str_starts_with($genderRaw, 'f')) { $gender = 'female'; }
        elseif ($genderRaw === 'm' || $genderRaw === 'boy' || str_starts_with($genderRaw, 'm')) { $gender = 'male'; }
        ...
        if ($ageYears === null && !empty($data['details']['age'])) {
            ...
            // Do not overwrite age string with integer, so frontend can parse "Year:Month"
            // $data['details']['age'] = $ageYears;
            ...
            if ($totalMonths !== null) {
                if ($totalMonths <= 60) { $wKey = $gender === 'female' ? 'wasting_birth_to_5_girl' : 'wasting_birth_to_5_boy'; }
                elseif ($totalMonths > 60 && $totalMonths <= 228) { $wKey = $gender === 'female' ? 'wasting_5_to_19_girl' : 'wasting_5_to_19_boy'; }
                
                if ($totalMonths < 24) { $sKey = $gender === 'female' ? 'stunting_birth_to_2_girl' : 'stunting_birth_to_2_boy'; }
                elseif ($totalMonths >= 24 && $totalMonths <= 60) { $sKey = $gender === 'female' ? 'stunting_2_5_girl' : 'stunting_2_5_boy'; }
                elseif ($totalMonths > 60 && $totalMonths <= 228) { $sKey = $gender === 'female' ? 'stunting_5_19_girl' : 'stunting_5_19_boy'; }
            } else {
            ...
            // Overwrite to match PDF primary behavior
