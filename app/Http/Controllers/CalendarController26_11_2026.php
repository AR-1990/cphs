    ...
    public function followupsByDateMedical(Request $request)
    {
        ...
        $gpRows = DB::table('school_health_physicians as shp')
            ...
            ->where(function($q) use ($date) {
                $ts = strtotime($date);
                $alt1 = $ts ? date('d-m-Y', $ts) : $date;
                $alt2 = $ts ? date('d/m/Y', $ts) : $date;
                $alt3 = $ts ? date('j F Y', $ts) : $date;
                $alt4 = $ts ? date('d F Y', $ts) : $date;
                $alt5 = $ts ? date('j M Y', $ts) : $date;
                $alt6 = $ts ? date('d M Y', $ts) : $date;
                $q->where('shp.Follow_up_Date', $date)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $date . '%')
                  ->orWhere('shp.Follow_up_Date', $alt1)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt1 . '%')
                  ->orWhere('shp.Follow_up_Date', $alt2)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt2 . '%')
                  ->orWhere('shp.Follow_up_Date', $alt3)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt3 . '%')
                  ->orWhere('shp.Follow_up_Date', $alt4)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt4 . '%')
                  ->orWhere('shp.Follow_up_Date', $alt5)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt5 . '%')
                  ->orWhere('shp.Follow_up_Date', $alt6)
                  ->orWhere('shp.Follow_up_Date', 'LIKE', $alt6 . '%');
            })
        ...
        $nutRows = DB::table('nutritionist_history_evaluation_sections as nhes')
            ...
            ->where(function($q) use ($date) {
                ...
                $alt5 = $ts ? date('j M Y', $ts) : $date;
                $alt6 = $ts ? date('d M Y', $ts) : $date;
                $q->where('nhes.Follow_up_Date1', $date)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $date . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt1)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt1 . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt2)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt2 . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt3)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt3 . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt4)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt4 . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt5)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt5 . '%')
                  ->orWhere('nhes.Follow_up_Date1', $alt6)
                  ->orWhere('nhes.Follow_up_Date1', 'LIKE', $alt6 . '%');
            })
        ...
        $psyRows = DB::table('psychologist_history_assessment_sections as phas')
            ...
            ->where(function($q) use ($date) {
                ...
                $alt5 = $ts ? date('j M Y', $ts) : $date;
                $alt6 = $ts ? date('d M Y', $ts) : $date;
                $q->where('phas.Follow_up_Date2', $date)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $date . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt1)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt1 . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt2)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt2 . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt3)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt3 . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt4)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt4 . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt5)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt5 . '%')
                  ->orWhere('phas.Follow_up_Date2', $alt6)
                  ->orWhere('phas.Follow_up_Date2', 'LIKE', $alt6 . '%');
            })
