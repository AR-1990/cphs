...
class ScreeningController extends Controller
{
   ...
   public function CreateUpdate(Request $request, $id = null)
   {
      ...
      if ($id > 0) {
         ...
         foreach ($keysInit as $k) {
            if (!array_key_exists($k, $data['details'])) {
               $data['details'][$k] = null;
            }
         }

         $genderRaw = trim(strtolower((string)($data['details']['gender'] ?? '')));
         $gender = $genderRaw;
         if ($genderRaw === 'f' || $genderRaw === 'girl' || str_starts_with($genderRaw, 'f')) {
            $gender = 'female';
         } elseif ($genderRaw === 'm' || $genderRaw === 'boy' || str_starts_with($genderRaw, 'm')) {
            $gender = 'male';
         }
         ...
         $weightVal = is_numeric($data['details']['Question_No_2_Weight'] ?? null) ? (float)$data['details']['Question_No_2_Weight'] : null;
         $bmiVal = null;
         if (is_numeric($data['details']['bmi61'] ?? null)) {
            $bmiVal = (float)$data['details']['bmi61'];
         } elseif (is_numeric($data['details']['Question_No_3_BMI'] ?? null)) {
            $bmiVal = (float)$data['details']['Question_No_3_BMI'];
         }
         ...
         if ($totalMonths === null && !empty($data['details']['age'])) {
            if (preg_match('/(\d+):(\d+)/', (string)$data['details']['age'], $matches)) {
               $y = (int)$matches[1];
               $m = (int)$matches[2];
               $totalMonths = ($y * 12) + $m;
               if ($ageYears === null) {
            ...
            } elseif (preg_match('/(\d+)/', (string)$data['details']['age'], $matches)) {
               $y = (int)$matches[1];
               $totalMonths = $y * 12;
               if ($ageYears === null) {
                  $ageYears = $y;
         ...
         if (in_array($gender, ['male', 'female']) && $totalMonths !== null) {
            if ($bmiVal !== null) {
               if ($totalMonths <= 60) {
                  ...
                  if ($wRow) {
                     $neg3 = (float)$wRow->Neg3SD;
                     $neg2 = (float)$wRow->Neg2SD;
                     $neg1 = (float)$wRow->Neg1SD;
                     $pos1 = (float)$wRow->Pos1SD;
                     $pos2 = (float)$wRow->Pos2SD;
                     $pos3 = (float)$wRow->Pos3SD;
                     $cls = '';
                     if ($bmiVal < $neg3) {
                        $cls = 'Severe Thinness';
                     } elseif ($bmiVal >= $neg3 && $bmiVal < $neg2) {
                        $cls = 'Moderate Thinness';
                     } elseif ($bmiVal >= $neg2 && $bmiVal < $neg1) {
                        $cls = 'Mild Thinness';
                     } elseif ($bmiVal >= $neg1 && $bmiVal <= $pos1) {
                        $cls = 'Normal Weight';
                     } elseif ($bmiVal > $pos1 && $bmiVal <= $pos2) {
                        $cls = 'Mild Overweight';
                     } elseif ($bmiVal > $pos2 && $bmiVal <= $pos3) {
                        $cls = 'Overweight';
                     } elseif ($bmiVal > $pos3) {
                        $cls = 'Obesity';
                     }
                     if ($gender === 'female') {
                        $data['details']['wasting_birth_to_5_girl'] = $cls;
                     } else {
                        $data['details']['wasting_birth_to_5_boy'] = $cls;
                     }
                  }
               } elseif ($totalMonths > 60 && $totalMonths <= 228) {
                  $bRow = DB::table('bmiForAge')->where('Sex', ucfirst($gender))->where('Months', $totalMonths)->first();
                  if ($bRow) {
                     $neg3 = (float)$bRow->Neg3SD;
                     $neg2 = (float)$bRow->Neg2SD;
                     $neg1 = (float)$bRow->Neg1SD;
                     $pos1 = (float)$bRow->Pos1SD;
                     $pos2 = (float)$bRow->Pos2SD;
                     $pos3 = (float)$bRow->Pos3SD;
                     $cls = '';
                     if ($bmiVal < $neg3) {
                        $cls = 'Severe Thinness';
                     } elseif ($bmiVal >= $neg3 && $bmiVal < $neg2) {
                     ...
                     if ($gender === 'female') {
                        $data['details']['wasting_5_to_19_girl'] = $cls;
                     } else {
                        $data['details']['wasting_5_to_19_boy'] = $cls;
                     }
                  }
               }
            }
            ...
            if ($heightVal !== null) {
               ...
               if ($zRow) {
                  $sd3n = (float)$zRow->SD3neg;
                  $sd2n = (float)$zRow->SD2neg;
                  $sd2p = (float)$zRow->SD2;
                  $cls = '';
                  if ($totalMonths > 60 && $totalMonths <= 228) {
                     if ($heightVal <= $sd3n) {
                        $cls = 'Severe Stunting';
                     } elseif ($heightVal > $sd3n && $heightVal <= $sd2n) {
                        $cls = 'Stunting';
                     } elseif ($heightVal > $sd2n && $heightVal <= $sd2p) {
                        $cls = 'Normal';
                     } elseif ($heightVal >= $sd2p) {
                        $cls = 'Tall';
                     }
                     if ($gender === 'female') {
                        $data['details']['stunting_5_19_girl'] = $cls;
                     } else {
                        $data['details']['stunting_5_19_boy'] = $cls;
                     }
                  } elseif ($totalMonths < 24) {
                     if ($heightVal <= $sd3n) {
                        $cls = 'Severe Stunting (LAZ < -3)';
                     } elseif ($heightVal > $sd3n && $heightVal <= $sd2n) {
                        $cls = 'Stunting (LAZ between -3 and -2)';
                     } elseif ($heightVal > $sd2n && $heightVal <= $sd2p) {
                        $cls = 'Normal (LAZ between -2 and +2)';
                     } elseif ($heightVal > $sd2p) {
                        $cls = 'Tall (LAZ > +2)';
                     }
                     if ($gender === 'female') {
                        $data['details']['stunting_birth_to_2_girl'] = $cls;
                     } else {
                        $data['details']['stunting_birth_to_2_boy'] = $cls;
                     }
                  } elseif ($totalMonths >= 24 && $totalMonths <= 60) {
                     if ($heightVal <= $sd3n) {
                        $cls = 'Severe Stunting (LAZ/HAZ < -3)';
                     } elseif ($heightVal > $sd3n && $heightVal <= $sd2n) {
                        $cls = 'Stunting (LAZ/HAZ between -3 and -2)';
                     } elseif ($heightVal > $sd2n && $heightVal <= $sd2p) {
                        $cls = 'Normal (LAZ/HAZ between -2 and +2)';
                     } elseif ($heightVal > $sd2p) {
                        $cls = 'Tall (LAZ/HAZ > +2)';
                     }
                     if ($gender === 'female') {
                        $data['details']['stunting_2_5_girl'] = $cls;
                     } else {
                        $data['details']['stunting_2_5_boy'] = $cls;
                     }
                  }
               }
            }
         ...
         if ($ageYears === null && !empty($data['details']['age'])) {
            if (preg_match('/(\d+)/', (string)$data['details']['age'], $matches)) {
               $ageYears = (int)$matches[1];
            }
         }

         $data['details']['total_months'] = $totalMonths;
         ...
         $map['wasting_birth_to_5_girl'] = [
            'Severe Thinness' => [
               'n' => 'The child is severely underweight, the z score indicates severe wasting which needs to be addressed immediately. It can also indicate any underlying medical condition causing this severe malnutrition. Ensure regular follow-ups with the health care provider',
         ...
         if ($totalMonths !== null) {
            if ($totalMonths <= 60) {
               $wKey = $gender === 'female' ? 'wasting_birth_to_5_girl' : 'wasting_birth_to_5_boy';
            } elseif ($totalMonths > 60 && $totalMonths <= 228) {
               $wKey = $gender === 'female' ? 'wasting_5_to_19_girl' : 'wasting_5_to_19_boy';
            }

            if ($totalMonths < 24) {
               $sKey = $gender === 'female' ? 'stunting_birth_to_2_girl' : 'stunting_birth_to_2_boy';
            } elseif ($totalMonths >= 24 && $totalMonths <= 60) {
               $sKey = $gender === 'female' ? 'stunting_2_5_girl' : 'stunting_2_5_boy';
            } elseif ($totalMonths > 60 && $totalMonths <= 228) {
               $sKey = $gender === 'female' ? 'stunting_5_19_girl' : 'stunting_5_19_boy';
            }
         } else {
         ...
         $data['details']['NutritionistComment'] = $computedNutrition;
         $data['details']['DietaryAdviceComment'] = $computedDiet;
      ...
      if ($request->isMethod('post')) {
         ...
         $enter_by = Auth::guard('admin')->user()->id;
           // Check for duration in direct input first, then fallback to loop
         $duration = $request->input('duration');
         if ($duration !== null) {
             Log::info("Controller received duration (direct input): " . $duration);
         }
         $bmiresult = null;
         $systolicresult = null;
         $diastolic = null;
         ...
         foreach ($formData as $field) {
            if ($field['name'] === 'name') {
               ...
               if ($duration === null) {
                   ...
                   Log::info("Controller received duration (from formData): " . $duration);
               }
         ...
         if ($currentStepNumber == 1) {
            ...
            // $form->duration = gmdate('H:i:s', (int)($duration ?? 0));
            // $form->duration = gmdate('i:s', $duration);
             $form->duration = gmdate('H:i:s', (int)$duration);
            $form->save();
            ...
            $message = 'Form data has been updated successfully';
               if ($duration !== null) {
                Log::info("Updating duration for ID $storedRecordId to " . gmdate('H:i:s', (int)$duration));
                form_entry::where('id', $storedRecordId)->update(['duration' => gmdate('H:i:s', (int)$duration)]);
             } else {
                Log::info("Duration is null for ID $storedRecordId");
             }
...
// dd($data);
      ...
      public function ScreeningList(Request $request)
   {
      ...
      if (!empty($fromDate) && !empty($toDate)) {
         ...
         $query->whereBetween('form_entries.created_at', [$start, $end]);
      }
...
foreach ($questions as $q) {
    $questionKeysList[] = $q['key'];
}

$formDataRows = [];
if (!empty($entryIds) && !empty($questionKeysList)) {
    $formDataRows = FormData::whereIn('entry_id', $entryIds)
        ->whereIn('key', $questionKeysList)
        ->get(['entry_id', 'key', 'value']);
}

$groupedFormData = [];
foreach ($formDataRows as $row) {
    $groupedFormData[$row->entry_id][$row->key][] = $row->value;
}

foreach ($entries as $entry) {

    $entryData = $groupedFormData[$entry->id] ?? [];
    ...
    foreach ($questions as $question) {
        ...
        $valuesForKey = $entryData[$key];
        $matchedValues = [];

        if (is_array($question['value'])) {
            $matchedValues = array_values(array_intersect($valuesForKey, $question['value']));
        } else {
            foreach ($valuesForKey as $v) {
                if ($v == $question['value']) {
                    $matchedValues[] = $v;
                }
            }
...
// dd($data['findings']);
      ...
      $data['medicalComplain'] = DB::table('medical_complains')->where('stdId', $id)->get()->toArray();
