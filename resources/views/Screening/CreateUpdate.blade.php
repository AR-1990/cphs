...
$("#gender").on("keyup change", function() {
    ...
    var years = today.getFullYear() - birthDate.getFullYear();
    var monthDiff = today.getMonth() - birthDate.getMonth();
    var totalMonths = years * 12 + monthDiff;
    if (today.getDate() < birthDate.getDate()) {
        totalMonths--;
    }

    var ageYears = totalMonths / 12;
    $('#read_oly_age').val(ageYears.toFixed(2));
    $('#read_oly_gender').val(gender);

    if (gender == 'female' && totalMonths >= 0 && totalMonths < 60) {
        $('#birth_5_wasting_girls').removeClass('d-none');
    } else if (gender == 'male' && totalMonths >= 0 && totalMonths < 60) {
    ...
    if (gender == 'female' && totalMonths >= 0 && totalMonths < 24) {
        $('#birth_2_stunting_girls').removeClass('d-none');
    } else if (gender == 'male' && totalMonths >= 0 && totalMonths < 24) {
...
$('.NutritionistOptionsAttribute').each(function() {
    ...
    $('#Question_No65_How_many_glasses_of_waterliquids_do_you_consume_in_a_day,#food_items,#Question_No66_Does_the_child_have_any_history_of_substances_abuse_or_addiction_to').on('change', function() {
        ...
        setDataValueForOptions1();
            ...
            // Sirf comments update karne ke liye
            function refreshNutritionComments() {
                console.log("refreshNutritionComments");
                const wastingSelectedOptions = $('.NutritionistSelectWasting')
                ...
                const wastingDiagnosis = wastingSelectedOptions
                    .map(function() {
                ...
                const wastingDietaryAdvice = wastingSelectedOptions
                    .map(function() {
                ...
                const stuntingSelectedOptions = $('.NutritionistSelectStunting')
                    .find('option:selected')
                    .filter(function() {
                        const val = $(this).val();
                ...
                const stuntingDiagnosis = stuntingSelectedOptions
                    .map(function() {
                        return $(this).attr('data-nutritional-diagnosis');
                    })
                    .get()
                ...
                const stuntingDietaryAdvice = stuntingSelectedOptions
                    .map(function() {
                        return $(this).attr('data-dietary-advice');
                    })
                    .get()
            ...
            function updateComments() {
                // Pehle current selections se comments sync karo
                refreshNutritionComments();
                        ...
                        success: function(response) {
                            ...
                            if (response ) {
                              ...
                              // } else if (bmi61 > Pos1SD) {
                                ...
                                // Auto-selected classification ke basis par comments refresh karo
                                refreshNutritionComments();
                               ...
                               const SD3neg = parseFloat(response.data.SD3neg);
                                ...
                                else if($("#read_oly_age").val() < 2)
                                {
                                    ...
                                    if (Question_No_1_Height <= SD3neg) {
                                        ...
                                        $(dropdownId).val('Severe Stunting (LAZ < -3)');
                                        $(dropdownId).attr('style', 'background-color: red !important; color: white !important;');
                                    }
                                    else if (Question_No_1_Height > SD3neg && Question_No_1_Height <= SD2neg) {
                                        console.log("Stunting (LAZ between -3 and -2)");
                                        $(dropdownId).val('Stunting (LAZ between -3 and -2)');
                                        $(dropdownId).attr('style', 'background-color: yellow !important; color: black !important;');
                                    }
                                    else if (Question_No_1_Height > SD2neg && Question_No_1_Height <= SD2) {
                                        console.log("Normal (LAZ between -2 and +2)");
                                        $(dropdownId).val('Normal (LAZ between -2 and +2)');
                                        $(dropdownId).attr('style', 'background-color: green !important; color: white !important;');
                                    }
                                    else if ( Question_No_1_Height > SD2) {
                                        console.log("Tall (LAZ > +2)");
                                        $(dropdownId).val('Tall (LAZ > +2)');
                                        $(dropdownId).attr('style', 'background-color: blue !important; color: white !important;');
                                    }
                                    else {
                                        console.log("Unknown Classification");
                                ...
                                else if($("#read_oly_age").val() >= 2 && $("#read_oly_age").val() <= 5)
                                {
                                    ...
                                    // no classification for this age range
                                }

                                // Auto-selected stunting classification ke basis par comments refresh karo
                                refreshNutritionComments();
            ...
            // Attach change events – sirf comments refresh, koi extra calculation nahi
  ...
  $("#Question_No_48_Frequently_put_things_in_mouth").on("keyup change", function() {
            ...
            $('.nextStep').on('click', function(e) {
                ...
                activeStepData.push({name: 'duration', value: duration});
