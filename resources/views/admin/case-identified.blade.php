            ...
            success: function(response) {
                ...
                Object.keys(categories).forEach(function(cat) {
                    ...
                    if (allStudents && allStudents.length > 0) {
                        ...
                        html += '<table class="table table-bordered mt-2"><thead><tr><th>School Name</th><th>ID</th><th>GR No</th><th>Name</th><th>Phone</th><th>Action</th></tr></thead><tbody>';
                        allStudents.forEach(function(stu) {
                        html += '<tr><td>' + (school.school_name || '') + '</td><td>' + (stu.id || '') + '</td><td>' + (stu.gr || '') + '</td><td>' + (stu.name || '') + '</td><td>' + (stu.phone || '') + '</td><td><a href="/admin/medical_history/Views/' + stu.id + '" target="_blank" class="btn btn-sm btn-info">View</a></td></tr>';
                        });
                        html += '</tbody></table>';
                    } else {
