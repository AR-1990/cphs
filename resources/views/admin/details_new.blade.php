                    ...
                    <div class="row">
                        ...
                        <div class="col-12 box_style">
                            ...
                            <div class="form-row">
                                ...
                                <script>
                                    (function(){
                                        function computeNormalizedAge(){
                                            var s = "{{ isset($renderAge) ? $renderAge : '' }}";
                                            if(!s){ return null; }
                                            var parts = s.split(':');
                                            var years = parseInt(parts[0]);
                                            var months = parseInt(parts[1] || '0');
                                            if(isNaN(years)){ return null; }
                                            var ageRaw = years + (isNaN(months)?0:(months/12));
                                            var base = Math.floor(ageRaw);
                                            var frac = ageRaw - base;
                                            return base + (frac >= 0.7 ? 1 : 0);
                                        }

                                        function updateSystolic(){
                                            var systolicEl = document.getElementById('Question_No_6_Blood_Pressure_Systolic');
                                            var statusEl = document.getElementById('Blood_Pressure_Systolic');
                                            if(!systolicEl || !statusEl){ return; }
                                            var systolic = parseInt(systolicEl.value);
                                            var ageNormalized = computeNormalizedAge();
                                            statusEl.textContent = '';
                                            statusEl.classList.remove('text-success','text-danger');
                                            var min = null, max = null;
                                            if(ageNormalized !== null){
                                                if (ageNormalized >= 10 && ageNormalized <= 11) { min = 110; max = 120; }
                                            ...
                                            if (min !== null && max !== null && !isNaN(systolic)) {
                                                ...
                                                else if (systolic > max) { statusEl.textContent = 'Elevated'; statusEl.classList.add('text-danger'); }
                                                else { statusEl.textContent = 'Normal'; statusEl.classList.add('text-success'); }
                                            }
                                        }

                                        function updateDiastolic(){
                                            var diastolicEl = document.getElementById('Question_No_6_Blood_Pressure_Diastolic');
                                            var statusEl = document.getElementById('Blood_Pressure_Diastolic');
                                            if(!diastolicEl || !statusEl){ return; }
                                            var diastolic = parseInt(diastolicEl.value);
                                            var ageNormalized = computeNormalizedAge();
                                            statusEl.textContent = '';
                                            statusEl.classList.remove('text-success','text-danger');
                                            var min = null, max = null;
                                            if(ageNormalized !== null){
                                                if (ageNormalized >= 10 && ageNormalized <= 11) { min = 70; max = 80; }
                                                else if (ageNormalized >= 1 && ageNormalized <= 4) { min = 50; max = 65; }
                                                else if (ageNormalized >= 5 && ageNormalized <= 9) { min = 65; max = 80; }
                                                else if (ageNormalized >= 12 && ageNormalized <= 13) { min = 70; max = 80; }
                                                else if (ageNormalized >= 14 && ageNormalized <= 80) { min = 70; max = 80; }
                                            }
                                            if (min !== null && !isNaN(diastolic)) {
                                                if (diastolic < min) { statusEl.textContent = 'LOW'; statusEl.classList.add('text-danger'); }
                                                else if (diastolic > max) { statusEl.textContent = 'Elevated'; statusEl.classList.add('text-danger'); }
                                                else { statusEl.textContent = 'Normal'; statusEl.classList.add('text-success'); }
                                            }
                                        }
                        ...
                        <div class="col-12 box_style">
                            <div class="form-row">
                               <div class="form-group col-md-12">
                                    ...
                                    <style>
                                        .va-red { background-color: #ffcccc !important; color: #cc0000 !important; }
                                    </style>
                                    <script>
                                        (function(){
                                            var redValues = [
                                            ...
                                            function applyExactColor(el){
                                                if(!el) return;
                                                var v = (el.value || '').trim();
                                                if (redValues.indexOf(v) !== -1) {
                                                    el.classList.add('va-red');
                                                }
                                            }
                                            applyExactColor(document.querySelector('input[name="Question_No_24_Visual_acuity_using_Snellen’s_chart"]'));
                                            applyExactColor(document.querySelector('input[name="Question_No_24B_Visual_acuity_using_Snellens_chart_left_eye"]'));
                                        })();
                                    </script>
                                <div class="form-group col-md-12">
                                    <div class="form-group">
                    ...
                    <div class="form-group col-md-12">
                        <div class="form-group">
                            ...
                            <option value="5" {{ in_array('5', $referTo) ? 'selected' : '' }}>General Physician</option>
                                ...
                                <div class="form-row" id="follow_up_show">
...
I am writing to inform you that UZMA of class 3A  visited the health room on 6 JAN 2026 and was assessed by 
DR NABIRA. During the evaluation, the following symptoms were observed: DENTAL CARIES.

We kindly request that you facilitate Uzma's referral to a pdentist for further evaluation and management. Please ensure that the student's parents/guardians are informed and involved in this process.
...
<script>
    $(document).ready(function() {
      ...
      var age = parseFloat("{{ $details['age'] ?? '' }}") || 0;
        // console.log("fresh age ",age);
        var gender = ($("#gender").val() || '').trim().toLowerCase();
        
        // Handle Age format "8:04" for visibility logic as well
        var ageRaw = "{{ $details['age'] ?? '' }}";
        var totalMonths = parseInt("{{ $details['total_months'] ?? 0 }}");
        
        if (totalMonths > 0) {
            // Used server-side calculated months
        } else {
            // Fallback calculation in JS
            var match = ageRaw.match(/(\d+):(\d+)/);
            if (match) {
                age = parseFloat(match[1]); // Use years for comparison
        ...
        //     age--;
        // }
        console.log("not fresh age ",age, "totalMonths", totalMonths);
        // Set calculated age to the readonly input
        //  $('#read_oly_age').val(age);
// console.write(a);
        // $('#read_oly_gender').val(gender);
        
        // Wasting Criteria Visibility: 0-60 months (<=5 years) vs >60 months (5-19 years)
        if (gender == 'female' && totalMonths <= 60) {
            $('#birth_5_wasting_girls').removeClass('d-none');
        } else if (gender == 'male' && totalMonths <= 60) {
            $('#birth_5_wasting_boys').removeClass('d-none');
        } else if (gender == 'female' && totalMonths > 60 && totalMonths <= 228) {
            $('#5_19_wasting_girls').removeClass('d-none');
        } else if (gender == 'male' && totalMonths > 60 && totalMonths <= 228) {
            $('#5_19_wasting_boys').removeClass('d-none');
        } else {}

        // Stunting Criteria Visibility
        if (gender == 'female' && totalMonths < 24) {
            $('#birth_2_stunting_girls').removeClass('d-none');
        } else if (gender == 'male' && totalMonths < 24) {
            $('#birth_2_stunting_boys').removeClass('d-none');
        } else if (gender == 'female' && totalMonths >= 24 && totalMonths <= 60) {
            $('#2_5_stunting_girls').removeClass('d-none');
        } else if (gender == 'male' && totalMonths >= 24 && totalMonths <= 60) {
...
<script>
        var dataOptionsAttributes1 = {
            wasting_birth_to_5_girl: {
                'Severe Thinness': { nutritionalDiagnosis: 'The child is severely underweight, the z score indicates severe wasting which needs to be addressed immediately. It can also indicate any underlying medical condition causing this severe malnutrition. Ensure regular follow-ups with the health care provider', dietaryAdvice: 'Increase calorie-dense foods: whole milk, nuts, seeds, cheese, peanut butter. Include protein-rich foods like eggs, chicken, fish, and lentils. Fortify meals with healthy fats. Use full fat dairy products Provide frequent small meals with snacks in between. Consult a pediatric dietitian for a tailored plan.' },
                'Moderate Thinness': { nutritionalDiagnosis: 'The child is underweight and at risk of health issues. The Z score indicates moderate wasting. Nutritional support and monitoring every 6 months are recommended.', dietaryAdvice: 'Add nutrient-dense foods like whole grains, dairy, and lean proteins. Increase intake of fruits and vegetables. Focus on healthy fats and oils for cooking. Encourage consistent meal patterns with 3 main meals and 2 snacks.' },
                'Mild Thinness': { nutritionalDiagnosis: 'The child is approaching underweight. Regular monitoring and preventive nutritional measures are advised.', dietaryAdvice: 'Incorporate balanced meals with complex carbohydrates like whole grains, oats rice and whole grain pasta. Add moderate amounts of healthy fats and proteins which includes all meats beans and lentils. Introduce fortified cereals and dairy products. - Encourage healthy snacking between meals, like yogurt or fruit.' },
                'Normal Weight': { nutritionalDiagnosis: 'The child has a healthy weight for age and height. Encourage balanced nutrition and regular physical activity to maintain health.', dietaryAdvice: 'Maintain a balanced diet with all food groups: carbohydrates, proteins, fats, fruits, and vegetables. - Limit sugary snacks and processed foods.- Encourage regular hydration with water.- Promote at least 60 minutes of daily physical activity.' },
                'Mild Overweight': { nutritionalDiagnosis: 'The child is approaching overweight. Promote healthy eating habits and active play to prevent further weight gain.', dietaryAdvice: 'Replace sugary beverages with water or milk. Limit processed and high-fat snacks; choose fresh fruits and whole foods. Reduce portion sizes without skipping meals. Encourage regular physical activity, such as outdoor play or sports.' },
                'Overweight': { nutritionalDiagnosis: 'The child is overweight, increasing the risk of obesity-related health issues. Monitoring of calorie consumption and physical activity is needed  Early intervention with dietary and activity changes is needed.', dietaryAdvice: '- Focus on portion control and regular meal timings. Prioritize high-fiber foods like whole grains, fruits, and vegetables. Avoid fried and sugary foods; use baking or steaming methods. Introduce fun physical activities to reduce sedentary habits.' },
                'Obesity': { nutritionalDiagnosis: 'The child has significant excess weight, posing a high risk of severe health problems. Comprehensive intervention is required, potentially involving healthcare professionals.', dietaryAdvice: 'Consult a pediatric dietitian for a tailored plan. Gradually reduce calorie intake while ensuring nutrient density. Eliminate sugary drinks and high-fat junk foods. Increase daily physical activity, aiming for structured sports or fitness routines. Involve the family in adopting healthier eating and lifestyle habits' }
            },
            wasting_birth_to_5_boy: {}, 
            wasting_5_to_19_girl: {}, 
            wasting_5_to_19_boy: {}, 
            stunting_5_19_girl: {
                'Severe Stunting': { nutritionalDiagnosis: 'Severe stunting related to prolonged inadequate nutrient intake and possible health complications as evidenced by height-for-age Z-score below -3.', dietaryAdvice: 'Prioritize calorie-dense, nutrient-rich foods like lean proteins, whole grains, dairy, and regular healthy snacks to support catch-up growth.' },
                'Stunting': { nutritionalDiagnosis: 'Stunting related to insufficient nutrient variety and caloric intake as evidenced by height-for-age Z-score between -3 and -2.', dietaryAdvice: 'Incorporate protein-rich foods (e.g., meat, beans) and micronutrients (e.g., iron, calcium) to encourage healthy growth.' },
                'Normal': { nutritionalDiagnosis: 'Normal growth supported by adequate nutrition as evidenced by height-for-age Z-score between -2 and +2.', dietaryAdvice: ' ' },
                'Tall': { nutritionalDiagnosis: 'Above-average height likely due to genetic factors and sufficient nutrient intake, as evidenced by height-for-age Z-score greater than +2.', dietaryAdvice: ' ' }
            },
            stunting_5_19_boy: {}, 
            stunting_2_5_girl: {
                'Severe Stunting (LAZ/HAZ < -3)': { nutritionalDiagnosis: 'Severe stunting related to chronic malnutrition and possible recurrent infections, as evidenced by height-for-age Z-score below -3.', dietaryAdvice: 'Emphasize nutrient-dense, high-protein foods like eggs, dairy, meats, fortified cereals, and calorie-dense snacks to support growth recovery.' },
                'Stunting (LAZ/HAZ between -3 and -2)': { nutritionalDiagnosis: 'Stunting related to suboptimal dietary intake and lack of dietary diversity, as evidenced by height-for-age Z-score between -3 and -2.', dietaryAdvice: 'Increase protein and iron-rich foods, such as beans, lean meats, leafy greens, and fortified cereals, to boost growth and development.' },
                'Normal (LAZ/HAZ between -2 and +2)': { nutritionalDiagnosis: 'Normal growth supported by adequate nutrient intake, as evidenced by height-for-age Z-score between -2 and +2.', dietaryAdvice: ' ' },
                'Tall (LAZ/HAZ > +2)': { nutritionalDiagnosis: 'Above-average height potentially related to genetic factors and adequate nutrition, as evidenced by height-for-age Z-score greater than +2.', dietaryAdvice: 'Ensure continued balanced nutrition with appropriate portions to support proportionate growth and energy needs.' }
            },
            stunting_2_5_boy: {}, 
            stunting_birth_to_2_girl: {
                'Severe Stunting (LAZ < -3)': { nutritionalDiagnosis: 'Severe stunting related to chronic inadequate nutrient intake and frequent infections, as evidenced by length-for-age Z-score below -3.', dietaryAdvice: 'Prioritize high-calorie, nutrient-dense foods, including fortified cereals, protein-rich sources, and essential fats, and consult with a pediatric nutritionist.' },
                'Stunting (LAZ between -3 and -2)': { nutritionalDiagnosis: 'Stunting related to inadequate dietary diversity and insufficient caloric intake, as evidenced by length-for-age Z-score between -3 and -2.', dietaryAdvice: 'Incorporate iron- and protein-rich foods, including meats, legumes, and leafy greens, to support growth and immunity.' },
                'Normal (LAZ between -2 and +2)': { nutritionalDiagnosis: 'Normal growth supported by balanced nutrient intake as evidenced by length-for-age Z-score between -2 and +2.', dietaryAdvice: 'Maintain a balanced diet that includes a variety of fruits, vegetables, grains, and proteins to support continued healthy growth.' },
                'Tall (LAZ > +2)': { nutritionalDiagnosis: 'Height above average likely supported by adequate nutrient intake and genetics, as evidenced by length-for-age Z-score greater than +2.', dietaryAdvice: 'Continue with a well-balanced diet, ensuring nutrients are proportionate to height and activity levels to maintain balanced growth.' }
            },
            stunting_birth_to_2_boy: {}
        };
        dataOptionsAttributes1.wasting_birth_to_5_boy = dataOptionsAttributes1.wasting_birth_to_5_girl;
        dataOptionsAttributes1.wasting_5_to_19_girl = dataOptionsAttributes1.wasting_birth_to_5_girl;
        dataOptionsAttributes1.wasting_5_to_19_boy = dataOptionsAttributes1.wasting_birth_to_5_girl;
        dataOptionsAttributes1.stunting_5_19_boy = dataOptionsAttributes1.stunting_5_19_girl;
        dataOptionsAttributes1.stunting_2_5_boy = dataOptionsAttributes1.stunting_2_5_girl;
        dataOptionsAttributes1.stunting_birth_to_2_boy = dataOptionsAttributes1.stunting_birth_to_2_girl;

        function computeNutritionComments() {
            var gender = ($("#gender").val() || '').trim().toLowerCase();
            
            var age = parseFloat("{{ $details['age'] ?? '' }}") || 0;
            console.log('compute'+age);
            var diag = [], diet = [];
            var wKeys;
            if (gender === 'female') {
                wKeys = age <= 5 ? ['wasting_birth_to_5_girl','wasting_5_to_19_girl'] : ['wasting_5_to_19_girl','wasting_birth_to_5_girl'];
            } else {
                wKeys = age <= 5 ? ['wasting_birth_to_5_boy','wasting_5_to_19_boy'] : ['wasting_5_to_19_boy','wasting_birth_to_5_boy'];
            }
            var sKeys;
            if (gender === 'female') {
                if (age < 2) sKeys = ['stunting_birth_to_2_girl','stunting_2_5_girl','stunting_5_19_girl'];
                else if (age >= 2 && age <= 5) sKeys = ['stunting_2_5_girl','stunting_birth_to_2_girl','stunting_5_19_girl'];
                else sKeys = ['stunting_5_19_girl','stunting_2_5_girl','stunting_birth_to_2_girl'];
            } else {
                if (age < 2) sKeys = ['stunting_birth_to_2_boy','stunting_2_5_boy','stunting_5_19_boy'];
                else if (age >= 2 && age <= 5) sKeys = ['stunting_2_5_boy','stunting_birth_to_2_boy','stunting_5_19_boy'];
                else sKeys = ['stunting_5_19_boy','stunting_2_5_boy','stunting_birth_to_2_boy'];
            }
            var wKeyUsed = null, wValUsed = null;
            for (var i=0;i<wKeys.length;i++){ var v = $('#'+wKeys[i]).val(); if (v && v.trim() !== '') { wKeyUsed = wKeys[i]; wValUsed = v.trim(); break; } }
            var sKeyUsed = null, sValUsed = null;
            for (var j=0;j<sKeys.length;j++){ var sv = $('#'+sKeys[j]).val(); if (sv && sv.trim() !== '') { sKeyUsed = sKeys[j]; sValUsed = sv.trim(); break; } }
            if (wKeyUsed && dataOptionsAttributes1[wKeyUsed] && dataOptionsAttributes1[wKeyUsed][wValUsed]) {
                diag.push(dataOptionsAttributes1[wKeyUsed][wValUsed].nutritionalDiagnosis);
                diet.push(dataOptionsAttributes1[wKeyUsed][wValUsed].dietaryAdvice);
            }
            if (sKeyUsed && dataOptionsAttributes1[sKeyUsed] && dataOptionsAttributes1[sKeyUsed][sValUsed]) {
                diag.push(dataOptionsAttributes1[sKeyUsed][sValUsed].nutritionalDiagnosis);
                diet.push(dataOptionsAttributes1[sKeyUsed][sValUsed].dietaryAdvice);
            }
            $("#NutritionistComment").val(diag.join(' '));
            $("#DietaryAdviceComment").val(diet.join('\n'));
        }

        (function runExactCalculations(){
            var dob = $('#dob').val();
            var bmi61 = parseFloat($('#bmi61').val());
            var height = parseFloat($('#height').val());
            var weight = parseFloat($('#weight').val());
            
            // Calculate BMI if missing
            if (isNaN(bmi61) && !isNaN(height) && !isNaN(weight) && height > 0) {
                 var hM = height / 100;
                 bmi61 = weight / (hM * hM);
                 bmi61 = parseFloat(bmi61.toFixed(2));
                 $('#bmi61').val(bmi61);
            }

            var gender = ($('#gender').val() || '').trim().toLowerCase();
            // if (!dob || !gender) { computeNutritionComments(); return; }
            ...
            if (dob) {
                ...
                if (!isNaN(d)) {
                    var t = new Date();
                    totalMonths = (t.getFullYear() - d.getFullYear())*12 + (t.getMonth() - d.getMonth());
                }
            }
            if (totalMonths <= 0) {
                 var ageRaw = "{{ $details['age'] ?? '' }}";
                 // Prioritize "Year:Month" format if available
                 var match = ageRaw.match(/(\d+):(\d+)/);
                 if (match) {
                     totalMonths = (parseInt(match[1]) * 12) + parseInt(match[2]);
                 } else {
                     var matchY = ageRaw.match(/(\d+)/);
                     if (matchY) { totalMonths = parseInt(matchY[1]) * 12; }
                 }
            }
            
            // Determine Age in Years for conditional logic
            var ageYears = parseFloat("{{ $details['age'] ?? '' }}") || 0;
            var ageRaw = "{{ $details['age'] ?? '' }}";
            var totalMonths = parseInt("{{ $details['total_months'] ?? 0 }}");
            ...
            if (totalMonths > 0) {
                ...
                var matchAge = ageRaw.match(/(\d+):(\d+)/);
                if (matchAge) {
                     // If format is 8:04
                     ageYears = parseFloat(matchAge[1]);
                     totalMonths = (parseInt(matchAge[1]) * 12) + parseInt(matchAge[2]);
                } else {
                     totalMonths = ageYears * 12;
                }
            }

            // console.log('Calculated totalMonths:', totalMonths, 'Gender:', gender, 'AgeYears:', ageYears);

            if (totalMonths <= 0 || !gender) { computeNutritionComments(); return; }

            // Ensure gender is consistent
            gender = gender.toLowerCase();

            // Check if we have BMI for 5-19 age group
            if (totalMonths > 60 && totalMonths <= 228) {
                if (isNaN(bmi61)) {
                    // console.log('BMI61 is missing for age > 5');
                    // Try to calculate BMI if height and weight are available (optional fallback)
                }
            }

            if (totalMonths <= 60) {
                $.post('{!! Route('WastingCalculation') !!}', {"_token":"{{ csrf_token() }}", gender: gender, months: totalMonths}, function(resp){
                    if (resp && resp.data) {
                        // console.log('data');
                        var Neg3SD=parseFloat(resp.data.Neg3SD),Neg2SD=parseFloat(resp.data.Neg2SD),Neg1SD=parseFloat(resp.data.Neg1SD),Pos1SD=parseFloat(resp.data.Pos1SD),Pos2SD=parseFloat(resp.data.Pos2SD),Pos3SD=parseFloat(resp.data.Pos3SD);
                        var id = gender==='female' ? '#wasting_birth_to_5_girl' : '#wasting_birth_to_5_boy';
                        var val='';
                        if (bmi61 < Neg3SD) val='Severe Thinness';
                        else if (bmi61 >= Neg3SD && bmi61 < Neg2SD) val='Moderate Thinness';
                        else if (bmi61 >= Neg2SD && bmi61 < Neg1SD) val='Mild Thinness';
                        else if (bmi61 >= Neg1SD && bmi61 <= Pos1SD) val='Normal Weight';
                        else if (bmi61 > Pos1SD && bmi61 <= Pos2SD) val='Mild Overweight';
                        else if (bmi61 > Pos2SD && bmi61 <= Pos3SD) val='Overweight';
                        else if (bmi61 > Pos3SD) val='Obesity';
                        
                        // Force update the input field even if val is empty, to clear it if calculation fails
                        $(id).val(val);
                        // Trigger change to update visibility if needed (though visibility depends on age/gender, not value)
                        $(id).trigger('change');
                        
                        computeNutritionComments();
                    } else { computeNutritionComments(); }
                });
            } else if (totalMonths > 60 && totalMonths <= 228) {
                // Ensure totalMonths is passed correctly
                $.post('{!! Route('WastingBMICalculation') !!}', {"_token":"{{ csrf_token() }}", gender: gender, totalMonths: totalMonths, height: height}, function(resp){
                    if (resp && resp.data) {
                        var Neg3SD=parseFloat(resp.data.Neg3SD),Neg2SD=parseFloat(resp.data.Neg2SD),Neg1SD=parseFloat(resp.data.Neg1SD),Pos1SD=parseFloat(resp.data.Pos1SD),Pos2SD=parseFloat(resp.data.Pos2SD),Pos3SD=parseFloat(resp.data.Pos3SD);
                        var id = gender==='female' ? '#wasting_5_to_19_girl' : '#wasting_5_to_19_boy';
                        var val='';
                        if (bmi61 < Neg3SD) val='Severe Thinness';
                        else if (bmi61 >= Neg3SD && bmi61 < Neg2SD) val='Moderate Thinness';
                        else if (bmi61 >= Neg2SD && bmi61 < Neg1SD) val='Mild Thinness';
            ...
            $.post('{!! Route('StuntingCriteria5') !!}', {"_token":"{{ csrf_token() }}", gender: gender, totalMonths: totalMonths}, function(resp){
                if (resp && resp.data) {
                    var SD3neg=parseFloat(resp.data.SD3neg),SD2neg=parseFloat(resp.data.SD2neg),SD2=parseFloat(resp.data.SD2);
                    var id=''; var val='';
                    // Use totalMonths instead of ageYears for precise categorization
                    // 5-19 years = 61-228 months
                    if (totalMonths > 60 && totalMonths <= 228) { id = gender==='female' ? '#stunting_5_19_girl' : '#stunting_5_19_boy'; if (height <= SD3neg) val='Severe Stunting'; else if (height > SD3neg && height <= SD2neg) val='Stunting'; else if (height > SD2neg && height <= SD2) val='Normal'; else if (height >= SD2) val='Tall'; }
                    else if (totalMonths < 24) { id = gender==='female' ? '#stunting_birth_to_2_girl' : '#stunting_birth_to_2_boy'; if (height <= SD3neg) val='Severe Stunting (LAZ < -3)'; else if (height > SD3neg && height <= SD2neg) val='Stunting (LAZ between -3 and -2)'; else if (height > SD2neg && height <= SD2) val='Normal (LAZ between -2 and +2)'; else if (height > SD2) val='Tall (LAZ > +2)'; }
                    else if (totalMonths >= 24 && totalMonths <= 60) { id = gender==='female' ? '#stunting_2_5_girl' : '#stunting_2_5_boy'; if (height <= SD3neg) val='Severe Stunting (LAZ/HAZ < -3)'; else if (height > SD3neg && height <= SD2neg) val='Stunting (LAZ/HAZ between -3 and -2)'; else if (height > SD2neg && height <= SD2) val='Normal (LAZ/HAZ between -2 and +2)'; else if (height > SD2) val='Tall (LAZ/HAZ > +2)'; }
                    if (id) { $(id).val(val); }
                    computeNutritionComments();
                } else { computeNutritionComments(); }
            });
        ...
        $('#wasting_birth_to_5_girl, #wasting_birth_to_5_boy, #wasting_5_to_19_girl, #wasting_5_to_19_boy, #stunting_birth_to_2_girl, #stunting_birth_to_2_boy, #stunting_2_5_girl, #stunting_2_5_boy, #stunting_5_19_girl, #stunting_5_19_boy').on('change keyup', computeNutritionComments);
</script>
