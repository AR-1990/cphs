...
<script>
        var dataOptionsAttributes1 = {
            wasting_birth_to_5_girl: {
                'Severe Thinness': { nutritionalDiagnosis: "The child is severely malnourished, the z score indicates severe wasting which needs to be addressed immediately. It can also indicate any underlying medical condition causing this severe malnutrition. Ensure regular follow-ups with the health care provider", dietaryAdvice: "Consult a pediatrician and pediatric dietitian to get a tailored plan according to the energy requirements. Prioritize high-calorie, nutrient-dense foods including high protein options like eggs, meat, dairy and lentils and consider supplementation to support weight gain and recovery" },
            ...
            stunting_5_19_girl: {
                ...
                'Normal': { nutritionalDiagnosis: "Normal growth supported by adequate nutrition as evidenced by height-for-age Z-score between -2 and +2.", dietaryAdvice: "Maintain a balanced diet with a variety of fruits, vegetables, whole grains, lean proteins, and dairy to support overall health." },
                'Tall': { nutritionalDiagnosis: "Above-average height likely due to genetic factors and sufficient nutrient intake, as evidenced by height-for-age Z-score greater than +2.", dietaryAdvice: "Continue with a balanced diet, adjusting portions to support growth and daily activity levels." }
            },
            stunting_5_19_boy: {
                'Severe Stunting': { nutritionalDiagnosis: "Severe stunting related to prolonged inadequate nutrient intake and possible health complications as evidenced by height-for-age Z-score below -3.", dietaryAdvice: "Prioritize calorie-dense, nutrient-rich foods like lean proteins, whole grains, dairy, and regular healthy snacks to support catch-up growth." },
                'Stunting': { nutritionalDiagnosis: "Stunting related to insufficient nutrient variety and caloric intake as evidenced by height-for-age Z-score between -3 and -2.", dietaryAdvice: "Incorporate protein-rich foods (e.g., meat, beans) and micronutrients (e.g., iron, calcium) to encourage healthy growth." },
            ...
            stunting_2_5_girl: {
                ...
                "Normal (LAZ/HAZ between -2 and +2)": { nutritionalDiagnosis: "Normal growth supported by adequate nutrient intake, as evidenced by height-for-age Z-score between -2 and +2.", dietaryAdvice: "Maintain a balanced diet with a variety of fruits, vegetables, whole grains, lean proteins, and dairy to sustain healthy growth." },
        ...
        (function runExactCalculations(){
            ...
            var d = new Date(dob); var t = new Date();
            if (isNaN(d)) { computeNutritionComments(); return; }
            var totalMonths = (t.getFullYear() - d.getFullYear())*12 + (t.getMonth() - d.getMonth());
            if (parseFloat("{{ $details['age'] ?? '' }}") <= 5) {
            ...
            } else if (parseFloat("{{ $details['age'] ?? '' }}") > 5 && parseFloat("{{ $details['age'] ?? '' }}") <= 19) {
            ...
            $.post('{!! Route('StuntingCriteria5') !!}', {"_token":"{{ csrf_token() }}", gender: gender, totalMonths: totalMonths}, function(resp){
                if (resp && resp.data) {
                    ...
                    var id=''; var val='';
                    var age=parseFloat("{{ $details['age'] ?? '' }}");
                    if (age >= 5 && age <= 19) { id = gender==='female' ? '#stunting_5_19_girl' : '#stunting_5_19_boy'; if (height <= SD3neg) val='Severe Stunting'; else if (height > SD3neg && height <= SD2neg) val='Stunting'; else if (height > SD2neg && height <= SD2) val='Normal'; else if (height >= SD2) val='Tall'; }
                    else if (age < 2) { id = gender==='female' ? '#stunting_birth_to_2_girl' : '#stunting_birth_to_2_boy'; if (height <= SD3neg) val='Severe Stunting (LAZ < -3)'; else if (height > SD3neg && height <= SD2neg) val='Stunting (LAZ between -3 and -2)'; else if (height > SD2neg && height <= SD2) val='Normal (LAZ between -2 and +2)'; else if (height > SD2) val='Tall (LAZ > +2)'; }
                    else if (age >= 2 && age <= 5) { id = gender==='female' ? '#stunting_2_5_girl' : '#stunting_2_5_boy'; if (height <= SD3neg) val='Severe Stunting (LAZ/HAZ < -3)'; else if (height > SD3neg && height <= SD2neg) val='Stunting (LAZ/HAZ between -3 and -2)'; else if (height > SD2neg && height <= SD2) val='Normal (LAZ/HAZ between -2 and +2)'; else if (height > SD2) val='Tall (LAZ/HAZ > +2)'; }
                    if (id) { $(id).val(val); }
                    computeNutritionComments();
                } else { computeNutritionComments(); }
            });
