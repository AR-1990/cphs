...
<div class="pt-32pt">
    <div class="d-flex flex-column flex-md-row align-items-center text-center text-sm-left mx-auto">
        ...
        <form class="fromtosearch" method="GET" action="{{route('admin.followupSummaryReport')}}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ count($types) + 3 }}" class="text-center">No data found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
<script>
    ...
    function exportToCSV(students, title, school) {
        ...
        students.forEach(stu => {
            ...
            let link = `https://cphs.biopharmainfo.net/Medical_Detail/${stu.entry_id}`;
            csv += `"${name}","${phone}","${link}"\n`;
        });
            ...
            if (students.length > 0) {
                ...
                students.forEach(stu => {
                    html += `<tr>
                        <td>${stu.name ?? ''}</td>
                        <td>${stu.phone ?? ''}</td>
                        <td><a href="https://cphs.biopharmainfo.net/Medical_Detail/${stu.entry_id}" target="_blank" class="btn btn-success btn-sm">Show Details</a></td>
                    </tr>`;
                });
                html += `</tbody></table>`;
            } else {
